// 轻量自研 Toast: 全局容器 + toast.success/error API
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <>
      ...
      {/* 固定右上角容器 */}
      <div
        className="pointer-events-none fixed top-5 right-5 z-[999] flex w-72 flex-col gap-2"
        role="status"
        aria-live="polite"
      >
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`pointer-events-auto flex items-start gap-2 rounded-xl border px-3.5 py-2.5 text-[13px] shadow-[0_8px_24px_rgba(0,0,0,0.12)] backdrop-blur-xl ${
              t.type === 'success'
